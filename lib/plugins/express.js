...
var ExpressPlugin = function(options) {
    ...
    this.middleware = function(req, res, next) {
        ...
        if(res.statusCode)
          desc = "statusCode: " + res.statusCode;
        ...
        self._options.auditLog.logEvent(
          ...
          method, // action
          path, // path
          self._options.label || null, // label
          self._options.object || null, // object
          desc // description
        );
    ...
    this.pathAllowed = function(path, method) {
        ...
        if(self._options.whiteListPaths.length) {
            ...
            for(i=0; i<self._options.whiteListPaths.length; i++) {
                ...
                var matched_path = path.match(rule.regex);

                if(matched_path) {
                    self._options.label = matched_path[1];
                    app.param('id', function(req, res, next, id) {
                      self._options.object = id;
                      next();
                    });
                    if(rule.methods) {
