...
function AuditLog() {
     ...
     *
     */
    this.logEvent = function( actor, origin, action, path, label, object, description ) {
        
        if(!actor && this._userId) actor = this._userId; // set the actor if it wasn't provided.
        
        var eventPackage = this.buildEventPackage(actor, new Date(), origin, action, path, label, object, description);
     ...
     */
    this.buildEventPackage = function(actor, date, origin, action, path, label, object, description) {
        var defaults = { actor: '', date: new Date(), origin: '', action: '',
            path: '', label: '', object: '', description: '' };
